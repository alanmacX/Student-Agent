...
export default function ChatView({ conversation, onConversationUpdate, onOpenSidebar }) {
  ...
  const { startStream, stopStream, isStreaming } = useSSEStream({
    ...
    onDone: () => {
      ...
      // Reload messages from server to get the saved version
      if (conversation) {
        fetchMessages(conversation.id).then(setMessages).catch(console.error);
      }
    },
  ...
  const handleSend = useCallback(() => {
    ...
    startStream(`/api/conversations/${conversation.id}/chat`, {
      message: input.trim(),
    });
