...
Three-stage design:
  ...
  Stage 0 — user-configurable conversation filter (this module, DB-driven):
    * whitelist/blacklist on conversation title
...
# ── User-configurable filter config (loaded from DB) ─────────────────────────
# Module-level cache, refreshed on each sync run via load_filter_config().
...
async def load_filter_config(db_path: str) -> None:
    """Load filter config from DB into module cache. Call once per sync run."""
    import aiosqlite
    try:
        ...
        if row:
            _filter_config["conv_mode"] = row["conv_mode"] or "all"
            _filter_config["conv_list"] = json.loads(row["conv_list_json"] or "[]")
            _filter_config["custom_include_kw"] = json.loads(row["custom_include_kw_json"] or "[]")
            _filter_config["custom_exclude_kw"] = json.loads(row["custom_exclude_kw_json"] or "[]")
            _filter_config["min_text_length"] = row["min_text_length"] or 5
    except Exception:
        pass  # keep defaults


def _passes_conversation_filter(title: Optional[str]) -> bool:
    """Stage 0: apply user-configured conversation whitelist/blacklist."""
    mode = _filter_config["conv_mode"]
    if mode == "all" or not _filter_config["conv_list"]:
        return True
    title_norm = (title or "").strip().lower()
    in_list = any(t.strip().lower() in title_norm or title_norm in t.strip().lower()
                  for t in _filter_config["conv_list"])
    if mode == "whitelist":
        return in_list
    if mode == "blacklist":
        return not in_list
...
def _passes_keyword_filter(text: Optional[str]) -> Optional[str]:
    """Stage 0: apply user custom keyword rules.
    Returns 'include' | 'exclude' | None (no match → continue normal flow).
    """
    t = (text or "").lower()
    for kw in _filter_config["custom_include_kw"]:
        if kw.strip().lower() in t:
            return "include"
    for kw in _filter_config["custom_exclude_kw"]:
        if kw.strip().lower() in t:
            return "exclude"
    return None
...
def evaluate(msg: Dict[str, Any]) -> Dict[str, Any]:
    """Stage-0 + Stage-1 evaluation.

    Stage 0: user conversation whitelist/blacklist + custom keyword rules.
    Stage 1: deterministic coarse filter (content type, category keywords).
    ...
    # ── Stage 0: user-configurable conversation + keyword filter ──────────
    if not _passes_conversation_filter(title):
        out = dict(msg)
        out.update({"verdict": DROP, "verdict_reason": "conversation filtered by user config",
                    "_keep": False, "_needs_llm": False})
        return out
    kw_action = _passes_keyword_filter(msg.get("text"))
    if kw_action == "exclude":
        out = dict(msg)
        out.update({"verdict": DROP, "verdict_reason": "keyword excluded by user config",
                    "_keep": False, "_needs_llm": False})
        return out

    # ── Stage 1: deterministic coarse filter ──────────────────────────────
    ...
    # User-defined include keyword overrides everything except system conversations
    if kw_action == "include" and not system_conv:
        verdict, reason = NOTIFY, "keyword included by user config"
    elif system_conv:
